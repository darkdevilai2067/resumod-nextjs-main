...
const SectionAts: React.FC = () => {
  ...
  return (
    <>
      <section className="py-28 mt-12 relative">
        <div className="container">
          ...
          <div className="md:grid md:grid-cols-3 gap-10 py-12 max-w-[1030px] mx-auto mt-20">
            {data.map((item, index) => (
              <div key={index} className="text-center">
                <Image src={item.icon
                } alt="" width={54} height={54} className="mx-auto mb-5" />
                <h4 className="text-xl font-bold tracking-[-0.144px] leading-[2.6rem] max-w-[30rem]">{item.title}</h4>
              </div>
            ))}
          </div>
        </div>

        <Image src="/images/dots2.svg" alt="" width={96} height={187} className="absolute left-0 bottom-64" />   
        <Image src="/images/wave.svg" alt="" width={129} height={80} className="absolute -right-5 top-0" />  
